...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        #生成网格：首先，在初始化时为每个输入特征生成一个样条网格。网格定义了样条函数的节点（knots），这些节点是样条函数的分段点。
...
#计算B样条基函数：在 b_splines 函数中，计算输入张量 x 对应的B样条基函数。B样条基函数是一组分段多项式函数，这些函数在不同区间上有不同的值。
    def b_splines(self, x: torch.Tensor):
        ...
        grid: torch.Tensor = ( # 形状为 (in_features, grid_size + 2 * spline_order + 1)
            self.grid
        )  # (in_features, grid_size + 2 * spline_order + 1)# 样条控制点
        x = x.unsqueeze(-1)
    ...
    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor):
        ...
        """
        #最小二乘法拟合样条系数：curve2coeff 函数通过最小二乘法求解样条系数，使得样条函数尽可能地拟合输入数据点。这个过程是将输入数据点映射到B样条基函数上，并找到使误差最小的样条系数。
        assert x.dim() == 2 and x.size(1) == self.in_features
        assert y.size() == (x.size(0), self.in_features, self.out_features)#包含了输入数据 x 对应的目标输出值
        # 计算 B-样条基函数
        A = self.b_splines(x).transpose(
            0, 1 # 形状为 (in_features, batch_size, grid_size + spline_order)
        )  # (in_features, batch_size, grid_size + spline_order)
        B = y.transpose(0, 1)  # (in_features, batch_size, out_features) # 形状为 (in_features, batch_size, out_features)#B 是目标输出张量
