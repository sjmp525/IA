...
class KANLinear(torch.nn.Module):
    ...
    def poset_kernel(self, x: torch.Tensor):
        assert x.dim() == 2 and x.size(1) == self.in_features

        x = x.unsqueeze(-1) - self.grid.to(x.device).unsqueeze(0)
        k = self.poset_k
        kernel = 1 / (1 + torch.exp(-k * x))
    ...
    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor):
        ...
        A = self.poset_kernel(x).transpose(0, 1)
        B = y.transpose(0, 1)
    ...
    def forward(self, x: torch.Tensor):
        ...
        kernel_output = F.linear(
            self.poset_kernel(x).view(x.size(0), -1),
