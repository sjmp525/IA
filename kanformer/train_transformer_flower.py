...
def main(args):
    ...
    for epoch in range(args.epochs):
        # train
        start_time = time.time()  # 记录每回合开始的时间
        ...
        end_time = time.time()  # 记录每回合结束的时间

        epoch_time = end_time - start_time  # 计算每回合的时间

        print('[epoch %d] train_loss: %.3f  train_acc: %.3f  val_accuracy: %.3f  epoch_time: %.2f seconds' % (
        epoch + 1, mean_loss, train_acc, val_acc, epoch_time))

        with open(os.path.join(save_path, "mlp5.txt"), 'a') as f:
            f.writelines(
                '[epoch %d] train_loss: %.3f  train_acc: %.3f  val_accuracy: %.3f  epoch_time: %.2f seconds\n' % (
                epoch + 1, mean_loss, train_acc, val_acc, epoch_time))

        if opt.tensorboard:
            tags = ["train_loss", "train_acc", "val_accuracy", "learning_rate", "epoch_time"]
            tb_writer.add_scalar(tags[0], mean_loss, epoch)
            tb_writer.add_scalar(tags[1], train_acc, epoch)
            tb_writer.add_scalar(tags[2], val_acc, epoch)
            tb_writer.add_scalar(tags[3], optimizer.param_groups[0]["lr"], epoch)
            tb_writer.add_scalar(tags[4], epoch_time, epoch)

        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), os.path.join(save_path, "NEWzishiying2.pth"))
