...
class KANLinear(torch.nn.Module):
    ...
    def neighborhood_function(self, x: torch.Tensor):
        assert x.dim() == 2 and x.size(1) == self.in_features

        x = x.unsqueeze(-1) - self.grid.unsqueeze(0)
        neighborhood = (x.abs() <= self.scale_kernel).float()
        neighborhood = neighborhood / (neighborhood.sum(dim=-1, keepdim=True) + 1e-6)

        assert neighborhood.size() == (
            x.size(0),
            self.in_features,
            self.grid_size + 1,
        )
        return neighborhood.contiguous()

    def curve2coeff(self, x: torch.Tensor, y: torch.Tensor):
        assert x.dim() == 2 and x.size(1) == self.in_features
        assert y.size() == (x.size(0), self.in_features, self.out_features)

        A = self.neighborhood_function(x).transpose(0, 1)
        B = y.transpose(0, 1)

        # Increased regularization term to avoid singular matrix
        reg = 1e-4 * torch.eye(A.size(-1), device=A.device)
        A_T = A.transpose(-1, -2)
        A_reg = A_T @ A + reg

        # Use pseudoinverse instead of lstsq
        A_pinv = torch.linalg.pinv(A_reg)
        solution = A_pinv @ (A_T @ B)
    ...
    def forward(self, x: torch.Tensor):
        ...
        kernel_output = F.linear(
            self.neighborhood_function(x).view(x.size(0), -1),
    ...
    def update_grid(self, x: torch.Tensor, margin=0.01):
        ...
        kernels = self.neighborhood_function(x)
