...
from models.TransUnet import TResUnet
...
def main():
    net = RSIGT(3,BLU.num_classes + 1).cuda()
...
def predict(net, pred_loader, pred_path, f_out=None):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        pred_name = os.path.join(pred_path, '%d_RSIGT.png' % vi)
    ...
    avg_F = F1.mean()
    mIoU = IoU.mean()
    print('F1 Score:0:%.2f 1：%.2f 2：%.2f 3：%.2f 4：%.2f 5：%.2f' % (F1[0],F1[1],F1[2],F1[3],F1[4],F1[5]))
